...
def struct_generator(input_psf,input_pdb,seltext,caps,ftype='xyz',ofile='qm_coords',rem=None,charge=None,mult=None,mm=False,qmmm_mode=None,zerocharge=None,ext_coords=None,params=None):
	'''
	...
	def xyz_janus(ext_mol=None):
		...
		for i in range(len(qm_region.atoms)):
			...
			atom_types.append([typeID,qm_type[i]])
		...
		atom_types.append([typeID,'HA3'])

		count += 1

		# Write the MM region to the $molecule section
		mm_seltext = 'not ('+seltext+'or '+zerocharge+')' #+' or name H1 H2 OH2)'
		#h2o_seltext = ' or (name H1 H2 OH2 and around 9.0 protein) or (name H1 H2 OH2 and around 9.0 name SOD CLA)'
		...
		for i in range(len(mm_region.atoms)):
			...
			atom_types.append([typeID,mm_type[i]])

			count += 1

		atom_types = {str(atom_types[i][0]):str(atom_types[i][1]) for i in range(len(atom_types))}
	...
	if ftype == 'qchem':
		...
		if qmmm_mode == 'janus':
			...
			for i in range(len(charge_list)):
				idx = -1*(i+1)
				try:
					eps,sig = vdW_params(atom_types[str(idx)],params)
					print('AtomType %s %.3f %s %s' % (idx,charge_list[i],sig,eps),file=fileOUT)
				except:
